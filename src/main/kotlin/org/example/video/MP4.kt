...
class MP4(override val inputFilePath: String): FFmpegConvertibleType {
    ...
    override fun convertTo(outputFilePath: String, ffmpegExecutablePath: String): ConversionResult {
        ...
        when (targetExtension) {
            ...
            "wmv" -> {
                // WMV typically uses Windows Media Video codecs (vc1, wmv2, wmv3)
                // This might require specific FFmpeg build configurations.
                // For simplicity, let's try a generic re-encode to wmv2/wma2
                command.add("-c:v")
                command.add("wmv2")
                command.add("-c:a")
                command.add("wmav2")
